...
class TraktImporter(object):
    ...
    def __extract_fields(movies):
        return [{
            ...
            'RatedDate': x['rated_at'] if ('rated_at' in x) else '',
...
def run():
    ...
    if importer.authenticate():
        ratings = importer.get_movie_list('ratings')
        history = importer.get_movie_list('history')
        df_ratings = pd.DataFrame(ratings)
        df_history = pd.DataFrame(history)
        df_history = df_history.drop(columns = ['RatedDate','Rating10'])
        df_full = df_history.merge(df_ratings[['imdbID','Rating10']],how='left', left_on='imdbID', right_on='imdbID')

        df_full.to_csv("trakt-exported-history.csv", index=False)
